...
>(({ className, ...props }, ref) => (
    <div
        ref={ref}
        className={cn(
            "w-fit rounded-lg border bg-card text-card-foreground shadow-sm p-5 cursor-pointer transition-transform transform hover:scale-105 duration-300 ease-in-out",
            className
        )}
        {...props}
    />
